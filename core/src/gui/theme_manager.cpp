...
bool ThemeManager::loadTheme(std::string path) {
    ...
    for (auto const& [param, val] : params) {
        ...
        if (param == "WaterfallBackground" || param == "ClearColor" || param == "FFTHoldColor" || param == "SquelchColor" || param == "ScannerSquelchColor") {
...
bool ThemeManager::applyTheme(std::string name) {
    ...
    for (auto const& [param, val] : params) {
        ...
        if (param == "SquelchColor") {
            decodeRGBA(val, ret);
            squelchColor = ImVec4((float)ret[0] / 255.0f, (float)ret[1] / 255.0f, (float)ret[2] / 255.0f, (float)ret[3] / 255.0f);
            continue;
        }

        if (param == "ScannerSquelchColor") {
            decodeRGBA(val, ret);
            scannerSquelchColor = ImVec4((float)ret[0] / 255.0f, (float)ret[1] / 255.0f, (float)ret[2] / 255.0f, (float)ret[3] / 255.0f);
            continue;
        }
